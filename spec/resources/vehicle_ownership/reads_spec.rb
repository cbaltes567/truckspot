require "rails_helper"

RSpec.describe VehicleOwnershipResource, type: :resource do
  describe "serialization" do
    let!(:vehicle_ownership) { create(:vehicle_ownership) }

    it "works" do
      render
      data = jsonapi_data[0]
      expect(data.id).to eq(vehicle_ownership.id)
      expect(data.jsonapi_type).to eq("vehicle_ownerships")
    end
  end

  describe "filtering" do
    let!(:vehicle_ownership1) { create(:vehicle_ownership) }
    let!(:vehicle_ownership2) { create(:vehicle_ownership) }

    context "by id" do
      before do
        params[:filter] = { id: { eq: vehicle_ownership2.id } }
      end

      it "works" do
        render
        expect(d.map(&:id)).to eq([vehicle_ownership2.id])
      end
    end
  end

  describe "sorting" do
    describe "by id" do
      let!(:vehicle_ownership1) { create(:vehicle_ownership) }
      let!(:vehicle_ownership2) { create(:vehicle_ownership) }

      context "when ascending" do
        before do
          params[:sort] = "id"
        end

        it "works" do
          render
          expect(d.map(&:id)).to eq([
                                      vehicle_ownership1.id,
                                      vehicle_ownership2.id,
                                    ])
        end
      end

      context "when descending" do
        before do
          params[:sort] = "-id"
        end

        it "works" do
          render
          expect(d.map(&:id)).to eq([
                                      vehicle_ownership2.id,
                                      vehicle_ownership1.id,
                                    ])
        end
      end
    end
  end

  describe "sideloading" do
    # ... your tests ...
  end
end
